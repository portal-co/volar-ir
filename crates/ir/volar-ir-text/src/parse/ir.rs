...
fn mk_var(raw: u32) -> IRVarId {
    IRVarId(raw)
}
fn mk_type(raw: u32) -> TypeId {
    TypeId(raw)
}
fn mk_storage(raw: u32) -> StorageId {
    StorageId(raw)
}
fn mk_block_id(raw: u32) -> IRBlockId {
    IRBlockId(raw)
}
...
fn read_block_target(lex: &mut Lexer) -> Result<IRBlockTargetId, ParseError> {
    ...
    match kw {
        ...
        "block" => {
            lex.expect_byte(b':')?;
            let n = lex.read_u32()?;
            Ok(IRBlockTargetId::Block(mk_block_id(n)))
        }
        "dyn" => {
            lex.expect_byte(b':')?;
            let v = lex.read_var()?;
            Ok(IRBlockTargetId::Dyn(mk_var(v)))
        }
...
fn parse_ir_terminator(kw: &str, lex: &mut Lexer) -> Result<IRTerminator, ParseError> {
    match kw {
        "jmp" => {
            ...
            let args = read_var_id_list(lex)?;
            Ok(IRTerminator::Jmp {
                target: IRBranchTarget::new(func, args),
            })
        }
        "jmp_cond" => {
            lex.expect_key("cond")?;
            ...
            Ok(IRTerminator::JumpCond {
                condition,
                then_target: IRBranchTarget::new(true_block, true_args),
                else_target: IRBranchTarget::new(false_block, false_args),
            })
        }
        "jmp_table" => {
...
fn parse_bir_stmt(kw: &str, lex: &mut Lexer) -> Result<BIrStmt, ParseError> {
    match kw {
        ...
        "one" => Ok(BIrStmt::One),
        "and" => {
            let a = mk_var(lex.read_var()?);
            let b = mk_var(lex.read_var()?);
            Ok(BIrStmt::And(a, b))
        }
        "or" => {
            let a = mk_var(lex.read_var()?);
        ...
        "oracle_bit" => {
            ...
            Ok(BIrStmt::OracleProjectedBit { call, bit })
        }
        "action_call" => {
            let name = lex.read_string()?;
            lex.expect_key("guard")?;
