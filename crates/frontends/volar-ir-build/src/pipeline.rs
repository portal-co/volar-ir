...
impl Pipeline {
    ...
    /// Start from LLVM bitcode (`.bc`) or assembly (`.ll`) via the
    /// call-preserving structural importer.
    #[cfg(feature = "llvm")]
    ...
    /// produces `is_circuit()` Volar IR when it succeeds.
    #[cfg(feature = "llvm")]
...
fn load_source(
    ...
    match source {
        ...
        #[cfg(feature = "llvm")]
        Source::LlvmDirect { path, entry } => {
            let bytes = std::fs::read(&path)?;
            let context = inkwell::context::Context::create();
            let input = if path.extension().and_then(|e| e.to_str()) == Some("ll") {
                volar_llvm_ir_import::ModuleInput::Assembly(std::str::from_utf8(&bytes)?)
            } else {
                volar_llvm_ir_import::ModuleInput::Bitcode(&bytes)
            };
            let (blocks, types) = volar_llvm_ir_import::import(
                &context,
                input,
                &entry,
