...
impl<'ctx> Importer<'ctx> {
    ...
    /// operations. A symbolic `memcpy` is an importer-generated byte loop:
    /// its dynamic condition remains real VAFFLE CFG for movfuscation's step
    /// circuit instead of being expanded to a fixed maximum byte count.
    fn translate_memory_intrinsic(
        ...
        match intrinsic {
            MemoryIntrinsic::Memset => {
                ...
                let MemoryIntrinsicLength::Constant(n_bytes) = length else {
                    return Err(ImportError::Unsupported(
                        "symbolic llvm.memset length is not supported".into(),
                    ));
                };
            ...
            MemoryIntrinsic::Memcpy | MemoryIntrinsic::Memmove => {
                ...
                match length {
                    ...
                    MemoryIntrinsicLength::Symbolic { bits } => {
                        ...
                        self.symbolic_memcpy_loop(fctx, &dest_bits, &src_bits, &bits)
                    }
                }
            }
        }
    ...
    fn symbolic_memcpy_loop(
        ...
        if length_bits.is_empty() {
            return Err(ImportError::Unsupported(
                "symbolic llvm.memcpy length must not be empty".into(),
            ));
        }
        ...
        let byte = self.dispatch_read(fctx, &src, self.byte_tid, 1)?;
    ...
    fn pointer_with_byte_offset(
        ...
        index.truncate(PTR_BITS);
        while index.len() < PTR_BITS {
            index.push(self.bc_const_at(fctx, block, false));
        }
        let stack_scale: Bits = (0..PTR_BITS)
    ...
    fn null_ptr_bits(&mut self, fctx: &mut FuncCtx<'ctx>, block: BlockId) -> Bits {
        let mut bits: Bits = (0..PTR_BITS - 1)
