...
and the VOLE weaver use. Evaluators and backends apply these segments before
any code runs. `VirtOutput::storage_access` is a non-serialized compatibility
sidecar which declares every generated bytecode/handler-slot storage ID
`ReadOnly`; a committed virtual program additionally declares its
pre-initialized commitment storage `ReadOnly`. Consumers that need these facts
retain the sidecar alongside the legacy IR value. Typed register files and
keyed-commitment key storage remain read-write.
